...
import com.fit2081.quiztracker.data.students.StudentsViewModel
...
class TeacherDashboard : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContent {
            // Initialise the StudentViewModel using ViewModelProvider with a factory
            ...
            // Initialise the QuizAttemptViewModel
                    ...
                    bottomBar = {
                        BottomAppBar(
                            ...
                            Row(
                                ...
                                IconButton(onClick = {
                                    navController.navigate(TeacherDashboardScreen.AddStudent.route)
                                }) {
                                    Icon(Icons.Filled.Add, contentDescription = "Add Student")
                                }
                                IconButton(onClick = {
                                    navController.navigate(TeacherDashboardScreen.ListStudents.route)
                                }) {
                                    Icon(Icons.AutoMirrored.Filled.List, contentDescription = "List Student")
                                }
                            }
                        }
                    }
...
fun NavHostTeacher(
    ...
    NavHost(
        navController = navController,
        startDestination = TeacherDashboardScreen.AddStudent.route,
        modifier = modifier
    ) {
        composable(TeacherDashboardScreen.AddStudent.route) {
            AddStudent(studentViewModel = studentViewModel)
        }

        composable(TeacherDashboardScreen.ListStudents.route) {
            ListStudents(studentsViewModel = studentViewModel, navController)
        }

        composable(TeacherDashboardScreen.StudentAttempts.route){
            val studentId = it.arguments?.getString("id")
